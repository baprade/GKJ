...
<div id="formulir" class="flex flex-col items-center grow">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr>
                                ...
                                <x-cms.th title="Tanggal Lahir" class=""/>
                                ...
                                <x-cms.th title="Tanggal Baptis" class=""/>
                                <x-cms.th title="Tempat Sidi" class="text-start"/>
                                <x-cms.th title="Tanggal Sidi" class=""/>
                                <x-cms.th title="Status Nikah" class="text-start"/>
                                <x-cms.th title="Anggota Kelompok" class="text-start"/>
                                <x-cms.th title="Anggota Gereja Baru" class="text-start"/>
                                <x-cms.th title="Alamat Gereja Baru" class="text-start"/>
                                <x-cms.th title="Sebab/Alasan Pindah" class="text-start"/>
                                <x-cms.th title="Alamat (Tempat Tinggal) Baru" class="text-start"/>
                                <x-cms.th title="Pengikut" class="text-start"/>
                                <x-cms.th title="Majelis Pembina Kelompok" class="text-start"/>
    ...
    @if ($items->isNotEmpty())
        @foreach ($items as $item)
                            <tr wire:key="{{ $item->id }}" class="border-t border-slate-200 hover:bg-slate-100 hover:text-black">
                                ...
                                <td class="px-2 text-sm">{{ $item->tempat_sidi }}</td>
                                <td class="text-xs text-center max-w-20 min-w-20">{{ $item->tanggal_sidi }}</td>
                                <td class="px-2 text-sm">{{ $item->status_nikah->title ?? '-' }}</td>
                                <td class="px-2 text-sm">{{ $item->kelompok }}</td>
                                <td class="px-2 text-sm">{{ $item->anggota_gereja_baru }}</td>
                                <td class="px-2 text-sm">{{ $item->alamat_gereja_baru }}</td>
                                <td class="px-2 text-sm">{{ $item->alasan_pindah }}</td>
                                <td class="px-2 text-sm">{{ $item->alamat_baru }}</td>
                                <td class="px-2 text-sm">{{ $item->pengikut }}</td>
                                <td class="px-2 text-sm">{{ $item->majelis_pembina_kelompok }}</td>
    ...
    @else
                            <tr>
                                <td colspan="20" class="py-5 text-sm font-light text-center border-t border-slate-200">- x -</td>
                            </tr>
    @endif
                        </tbody>
                    </table>
