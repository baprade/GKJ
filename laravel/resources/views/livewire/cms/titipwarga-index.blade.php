...
<div id="formulir" class="flex flex-col items-center grow">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr>
                                ...
                                <x-cms.th title="Alamat Baru" class="text-start"/>
                                <x-cms.th title="Keterangan lain-lain" class="text-start"/>
                                <x-cms.th title="Sekretaris Kelompok" class="text-start"/>
    ...
    @if ($items->isNotEmpty())
        @foreach ($items as $item)
                            <tr wire:key="{{ $item->id }}" class="border-t border-slate-200 hover:bg-slate-100 hover:text-black">
                                ...
                                <td class="px-2 text-sm">{{ $item->tanggal_baptis }}</td>
                                <td class="px-2 text-sm">{{ $item->tempat_sidi }}</td>
                                <td class="px-2 text-sm">{{ $item->tanggal_sidi }}</td>
                                <td class="px-2 text-sm">{{ $item->alamat_baru }}</td>
                                <td class="px-2 text-sm">{{ $item->keterangan_lain }}</td>
                                <td class="px-2 text-sm">{{ $item->sekretaris_kelompok }}</td>
    ...
    @else
                            <tr>
                                <td colspan="17" class="py-5 text-sm font-light text-center border-t border-slate-200">- x -</td>
                            </tr>
    @endif
                        </tbody>
                    </table>
